...
## If Vercel still shows old lint errors
If Vercel logs still show `<img>` and `any` errors, it is usually deploying an older commit.

Checklist:
1. Confirm the deployment is using commit that contains:
   - `Image` import in `src/app/page.tsx`
   - `GeoFeature` type in `src/components/map/goa-map.tsx`
2. Clear Vercel build cache and redeploy.
3. Ensure Vercel uses pnpm (this repo includes `vercel.json` with `pnpm install`/`pnpm build`).
